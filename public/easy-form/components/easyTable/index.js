...
const Component = (props, ref) => {
    ...
    const dataScopeService = props.scopeService;
    const columnScopeService = scopeService;
    ...
    const updateDataSource = (newDataSource) => {
        ...
        const pagination = dataScopeService && typeof dataScopeService.getPageInfo === 'function'
            ? dataScopeService.getPageInfo()
            : {};
    ...
    const getDataTableColumn = async () => {
        ...
        if (props.columnsCode) {
            const response = await columnScopeService.getDataTableColumn(props.columnsCode, props.enableMockMode, props.columnsDetailApiPrefix);
                ...
                if (defaultDateRangeName) {
                    ...
                    if (searchFormUnitConfig && searchFormUnitConfig.transformResult && lodash.isArray(searchFormUnitConfig.transformResult)) {
                        ...
                        if (startDateName && endDateName) {
                            ...
                            dataScopeService && dataScopeService.updateSearch && dataScopeService.updateSearch({
    ...
    const componentUninstall = () => {
        columnScopeService && columnScopeService.updatePageInfo && columnScopeService.updatePageInfo(initialPagination);
    };
    const init = async () => {
        const tableId = v4();
        setTableId(tableId);
    ...
    const loadDataList = async (resetPage = false) => {
        var _a;
        if (dataScopeService && !props.getDataListApi) {
            const pagination = dataScopeService.getPageInfo ? dataScopeService.getPageInfo() : {};
        ...
        const data = await props.getDataListApi.call(dataScopeService) || [];
        const pagination = dataScopeService && dataScopeService.getPageInfo ? dataScopeService.getPageInfo() : { total: data.length };
    ...
    const resetPagination = () => {
        ...
        dataScopeService && dataScopeService.updatePageInfo && dataScopeService.updatePageInfo({
            current: 1,
            pageSize: 10,
            total: undefined
        });
    ...
    return (_jsxs("div", { className: `easy-table-component-container ${displayColumnSetting ? 'setting-column' : ''}`, children: [props.hideActionBar ? void 0 : (_jsxs("div", { className: 'table-action-bar', children: [_jsx("div", { className: 'spacer' }), _jsx("div", { className: 'actions', children: propsActionBarMenus })] })), easyTableColumnSetting, _jsxs("div", { className: 'table-wrap', children: [_jsxs(Table, { id: `table_id_${tableId}`, ref: currentTableRef, size: 'small', scroll: props.containerScroolMaxWidth ? { x: props.containerScroolMaxWidth } : {}, loading: loading, ...rowSelectionProp(), dataSource: dataSource, pagination: pagination, rowKey: rowKeyProps, onChange: (newPagination) => {
                            ...
                            dataScopeService && dataScopeService.updatePageInfo && dataScopeService.updatePageInfo(newPagination);
