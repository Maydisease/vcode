        ...
        return new Promise((resolve, reject) => {
            easyFormInst.$submit().then(async (values) => {
                if (values && values.tableTitle) {
                    ...
                    const scopeService = lodash.get(props, 'data.scopeService', lodash.get(props, 'scopeService', undefined));
                    const modifyTitle = lodash.get(props, 'data.modifyTitle', undefined) ||
                        lodash.get(scopeService, 'modifyTitle', undefined);
                    ...
                    const result = await modifyTitle.call(scopeService, tableTitle);
                    if (result) {
                        resolve(tableTitle);
                    }
